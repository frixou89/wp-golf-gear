...
/**
 * Golf_Calculator_Admin Class
 *
 * @class Golf_Calculator_Admin
 * @version	1.0.0
 * @since 1.0.0
 * @package	Golf_Calculator
 * @author Jeffikus
 */
final class Golf_Calculator_Admin {
	/**
	 * Golf_Calculator_Admin The single instance of Golf_Calculator_Admin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main Golf_Calculator_Admin Instance
	 *
	 * Ensures only one instance of Golf_Calculator_Admin is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main Golf_Calculator_Admin instance
	 */
	...
	public function register_settings_screen () {
		$this->_hook = add_submenu_page( 'options-general.php', __( 'Golf Calculator Settings', 'golfcp' ), __( 'Golf Calculator', 'golfcp' ), 'manage_options', 'golf-calculator', array( $this, 'settings_screen' ) );
	...
	public function register_settings () {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				register_setting( 'golf-calculator-settings-' . sanitize_title_with_dashes( $k ), 'golf-calculator-' . $k, array( $this, 'validate_settings' ) );
				add_settings_section( sanitize_title_with_dashes( $k ), $v, array( $this, 'render_settings' ), 'golf-calculator-' . $k, $k, $k );
			}
		}
	...
	public function render_settings ( $args ) {
		$token = $args['id'];
		$fields = Golf_Calculator()->settings->get_settings_fields( $token );
		...
		if ( 0 < count( $fields ) ) {
			foreach ( $fields as $k => $v ) {
				...
				add_settings_field( $k, $v['name'], array( Golf_Calculator()->settings, 'render_field' ), 'golf-calculator-' . $token , $v['section'], $args );
			}
		}
	} // End render_settings()
	...
	public function validate_settings ( $input ) {
		...
		return Golf_Calculator()->settings->validate_settings( $input, $tab );
	...
	public function get_admin_header_html ( $sections, $title ) {
		$defaults = array(
							'tag' => 'h2',
							'atts' => array( 'class' => 'golf-calculator-wrapper' ),
							'content' => $title
						);
	...
	private function _get_admin_header_data ( $sections, $title ) {
		$response = array( 'tag' => 'h2', 'atts' => array( 'class' => 'golf-calculator-wrapper' ), 'content' => $title );
		...
		if ( is_array( $sections ) && 1 < count( $sections ) ) {
			...
			foreach ( $sections as $key => $value ) {
				...
				$response['content'] .= '<a href="' . admin_url( 'options-general.php?page=golf-calculator&tab=' . sanitize_title_with_dashes( $key ) ) . '" class="' . esc_attr( $class ) . '">' . esc_html( $value ) . '</a>';
			}
		}

		return (array)apply_filters( 'golf-calculator-get-admin-header-data', $response );
