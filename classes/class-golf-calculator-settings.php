...
/**
 * Golf_Calculator_Settings Class
 *
 * @class Golf_Calculator_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	Golf_Calculator
 * @author Jeffikus
 */
final class Golf_Calculator_Settings {
	/**
	 * Golf_Calculator_Admnin The single instance of Golf_Calculator_Admnin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main Golf_Calculator_Settings Instance
	 *
	 * Ensures only one instance of Golf_Calculator_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main Golf_Calculator_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool)apply_filters( 'golf-calculator-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'golf-calculator-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'golf-calculator-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= Golf_Calculator()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'golf-calculator-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'golf-calculator-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'Standard Fields', 'golfcp' );
		$settings_sections['special-fields'] = __( 'Special Fields', 'golfcp' );
		...
		return (array)apply_filters( 'golf-calculator-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			case 'standard-fields':
				...
				$settings_fields['text'] = array(
												'name' => __( 'Example Text Input', 'golfcp' ),
												'type' => 'text',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'golfcp' )
											);
				$settings_fields['textarea'] = array(
												'name' => __( 'Example Textarea', 'golfcp' ),
												'type' => 'textarea',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'golfcp' )
				...
				$settings_fields['checkbox'] = array(
												'name' => __( 'Example Checkbox', 'golfcp' ),
												'type' => 'checkbox',
												'default' => '',
												'section' => 'standard-fields',
												'description' => __( 'Place the field description text here.', 'golfcp' )
				...
				$settings_fields['radio'] = array(
												'name' => __( 'Example Radio Buttons', 'golfcp' ),
												'type' => 'radio',
												'default' => '',
												'section' => 'standard-fields',
												'options' => array(
																	'one' => __( 'One', 'golfcp' ),
																	'two' => __( 'Two', 'golfcp' ),
																	'three' => __( 'Three', 'golfcp' )
															),
												'description' => __( 'Place the field description text here.', 'golfcp' )
											);
				$settings_fields['select'] = array(
													'name' => __( 'Example Select', 'golfcp' ),
													'type' => 'select',
													'default' => '',
													'section' => 'standard-fields',
													'options' => array(
			...
			case 'special-fields':
				...
				$settings_fields['select_taxonomy'] = array(
													'name' => __( 'Example Taxonomy Selector', 'golfcp' ),
		...
		return (array)apply_filters( 'golf-calculator-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'golf-calculator-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'golf-calculator-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'golf-calculator-' . $v, array() );
