...
/**
 * Golf Calculator Taxonomy Class
 *
 ...
 * @since 1.0.0
 */
class Golf_Calculator_Taxonomy {
	/**
	...
	public function __construct ( $post_type = 'thing', $token = 'thing-category', $singular = '', $plural = '', $args = array() ) {
		...
		if ( '' == $this->singular ) $this->singular = __( 'Category', 'golfcp' );
		if ( '' == $this->plural ) $this->plural = __( 'Categories', 'golfcp' );
	...
	private function _get_default_labels () {
		return array(
			    'name'                => sprintf( _x( '%s', 'taxonomy general name', 'golfcp' ), $this->plural ),
			    'singular_name'       => sprintf( _x( '%s', 'taxonomy singular name', 'golfcp' ), $this->singular ),
			    'search_items'        => sprintf( __( 'Search %s', 'golfcp' ), $this->plural ),
			    'all_items'           => sprintf( __( 'All %s', 'golfcp' ), $this->plural ),
			    'parent_item'         => sprintf( __( 'Parent %s', 'golfcp' ), $this->singular ),
			    'parent_item_colon'   => sprintf( __( 'Parent %s:', 'golfcp' ), $this->singular ),
			    'edit_item'           => sprintf( __( 'Edit %s', 'golfcp' ), $this->singular ),
			    'update_item'         => sprintf( __( 'Update %s', 'golfcp' ), $this->singular ),
			    'add_new_item'        => sprintf( __( 'Add New %s', 'golfcp' ), $this->singular ),
			    'new_item_name'       => sprintf( __( 'New %s Name', 'golfcp' ), $this->singular ),
			    'menu_name'           => sprintf( __( '%s', 'golfcp' ), $this->plural )
			  );
